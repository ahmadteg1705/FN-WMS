...
<div class="flex justify-between items-center mb-6">
    <div>
        ...
        <p class="text-gray-500">
            Perbarui data Tim Teknisi Fahasa Net
        </p>
    </div>
    ...
    <form action="{{ route('teams.update', $team->id) }}" method="POST">
                ...
                <select
                    ...
                    @foreach($technicians as $technician)
                        <option
                            ...
                            {{ old('leader', $team->leader) == $technician->user->name ? 'selected' : '' }}>
        ...
        <div class="mt-6">
            ...
            <textarea
                name="keterangan"
                rows="4"
                class="w-full border @error('keterangan') border-red-500 @else border-gray-300 @enderror rounded-lg px-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('keterangan', $team->keterangan) }}</textarea>
            @error('keterangan')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
