...
<div class="space-y-6 w-full">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-slate-800">Edit ODP</h1>
            <p class="text-sm text-slate-500 mt-0.5">Ubah data dan konfigurasi ODP {{ $odp->nama }}</p>
        </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="relative">
                            <select
                                ...
                                @foreach($routers as $router)
                                    <option
                                        value="{{ $router->nama }}"
                                        {{ old('router', $odp->router) == $router->nama ? 'selected' : '' }}>
                                        {{ $router->nama }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="card"
                            name="card"
                            value="{{ old('card', $odp->card) }}"
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all">
                    </div>

                    {{-- Kapasitas ONU (Readonly / Auto Calculate) --}}
                    <div>
                        ...
                        <input
                            type="number"
                            id="kapasitas"
                            value="{{ ($odp->onu_akhir - $odp->onu_awal) + 1 }}"
                            readonly
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="onu_awal"
                            name="onu_awal"
                            value="{{ old('onu_awal', $odp->onu_awal) }}"
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all">
                    </div>
                    ...
                    <div>
                        ...
                        <input
                            ...
                            id="onu_akhir"
                            name="onu_akhir"
                            value="{{ old('onu_akhir', $odp->onu_akhir) }}"
                            class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all">
                    </div>
                    {{-- Latitude --}}
                    <div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select
                                ...
                                name="status"
                                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all appearance-none pr-10">
                                <option value="1" {{ old('status', $odp->status) == 1 ? 'selected' : '' }}>
                                    🟢 Aktif
                                </option>
                                <option value="0" {{ old('status', $odp->status) == 0 ? 'selected' : '' }}>
                                    🔴 Nonaktif
                                </option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center px-3.5 pointer-events-none text-slate-400">
                ...
                <div>
                    ...
                    <textarea
                        ...
                        rows="4"
                        class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-700 placeholder-slate-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-600 transition-all"
                        placeholder="Catatan tambahan mengenai ODP ini...">{{ old('keterangan', $odp->keterangan) }}</textarea>
                </div>
...
{{-- SCRIPT JAVASCRIPT UNTUK HITUNG KAPASITAS DINAMIS --}}
<script>
